...
router.get('/authors', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM authors ORDER BY date_created DESC; SELECT count(*) as count FROM authors',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/authors', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO authors (fname, lname, age) VALUES ('${first_name}', '${last_name}', '${age}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/authors/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, fname, lname, age FROM authors WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/authors/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const last_name = req.body.last_name;
        const age = req.body.age;
        client.query(`UPDATE authors SET fname = '${first_name}', lname = '${last_name}', age = '${age}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/authors/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM authors WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/books', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM books ORDER BY date_created DESC; SELECT count(*) as count FROM books',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/books', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO books (title, description, isbn, length, author_id, category_id, publisher_id) VALUES ('${title}','${description}','${isbn}','${length}','${author_id}','${category_id}','${publisher_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/books/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, title, description, isbn, length, author_id, category_id, publisher_id FROM books WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/books/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const category_id = req.body.category_id;
        const publisher_id = req.body.publisher_id;
        client.query(`UPDATE books SET title = '${title}', description = '${description}', isbn = '${isbn}', length = '${length}', author_id = '${author_id}', category_id = '${category_id}', publisher_id = '${publisher_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/books/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM books WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/bookslenders', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM bookslenders ORDER BY date_created DESC; SELECT count(*) as count FROM bookslenders',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/bookslenders', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO bookslenders (date_returned, due_date, fine, book_id, lender_id) VALUES ('${date_returned}','${due_date}','${fine}','${book_id}','${lender_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/bookslenders/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, date_returned, due_date, fine, book_id, lender_id FROM bookslenders WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/bookslenders/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const book_id = req.body.book_id;
        const lender_id = req.body.lender_id;
        client.query(`UPDATE bookslenders SET date_returned = '${date_returned}', due_date = '${due_date}', fine = '${fine}', book_id = '${book_id}', lender_id = '${lender_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/bookslenders/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM bookslenders WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/categories', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM categories ORDER BY date_created DESC; SELECT count(*) as count FROM categories',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/categories', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const description = req.body.description;
            client.query(`INSERT INTO categories (name, description) VALUES ('${name}','${description}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/categories/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, name, description FROM categories WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/categories/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const name = req.body.name;
        const description = req.body.description;
        client.query(`UPDATE categories SET name = '${name}', description = '${description}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/categories/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM categories WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/lenders', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM lenders ORDER BY date_created DESC; SELECT count(*) as count FROM lenders',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/lenders', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO lenders (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/lenders/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM lenders WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/lenders/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        client.query(`UPDATE lenders SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
...
router.delete('/lenders/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM lenders WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/publishers', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM publishers; SELECT count(*) as count FROM publishers',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/publishers', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            sanitizeBody('name').trim().escape();
            const name = req.body.name;
            client.query(`INSERT INTO publishers (name) VALUES ('${name}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/publishers/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, name FROM publishers WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/publishers/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        sanitizeBody('name').trim().escape();
        const name = req.body.name;
        client.query(`UPDATE publishers SET name = '${name}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/publishers/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM publishers WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/staffs', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM staffs ORDER BY date_created DESC; SELECT count(*) as count FROM staffs',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/staffs', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const gender_id = req.body.gender_id;
            const address_id = req.body.address_id;
            client.query(`INSERT INTO staffs (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/staffs/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM staffs WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/staffs/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        client.query(`UPDATE staffs SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
        email = '${email}', phone_number = '${phone_number}', gender_id = '${gender_id}', address_id = '${address_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
...
router.delete('/staffs/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM staffs WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
